...
\begin{definition}[Signal Space]
    \(\DX(\Omega, \mathcal{C}) := \{x: \Omega \to \mathcal C\}\). Induces linear structure.
\end{definition}
...
\begin{definition}[Representation]
    Actions of \(\G\) via \(\rho: \G \to \R^{n \times n}\), assigning invertible \(\rho(\gg)\) w/ \(\rho(\gg\hg) = \rho(\gg) \rho(\hg)\). \(\rho(\gg)x(u) = x(\gg^{-1} u)\), \((\rho(\gg)(\rho(\hg)x))(u) = (\rho(\gg\hg)x)(u)\).
\end{definition}
...
\textbf{Arch} & \textbf{Dom} & \textbf{Symm} & \textbf{Arch} & \textbf{Dom} & \textbf{Symm} \\
\hline
CNN & Grid & Trans. & Spher. CNN & Sphere / SO(3) & Rot. SO(3) \\
Deep Sets & Set & Perm $\Sigma_n$ & GNN & Graph & Perm $\Sigma_n$ \\
Transf. & Com. Gr. & Perm $\Sigma_n$ & LSTM & 1D Grid & Time Warp \\
\end{tabular}
}
...
\begin{definition}[Incidence]
    \(\B \in \R^{|E_o| \times |V|}\), \(e = (i, j) \in E_o\): \(b_{e, v} = 1\) if \(v = i\), \(-1\) if \(v = j\); el \(0\).
\end{definition}

\begin{definition}[Deg]
    \(\DM = \text{diag}(d_i)\) w/ \(d_i = \sum_j a_{ij}\).
    \textbf{Edge Weight}:
    \(\A_e = \text{diag}(a_e)\) w/ \(a_e = a_{ij}\).
\end{definition}

\begin{lemma}
    \begin{itemize*}
        \item \(\B^\top \A_e \B = \DM - \A\)
        \item \(G\) {\color{H1}unweight}. \(\iff\) \(\A_e = \I\)
        \item \(G\) undir. \(\iff\) \(\A = \A^\top\).
    \end{itemize*}
\end{lemma}

\begin{definition}[In-/Equivar. for {\color{H1}UWG}]
    \(f(\PM\X, \PM\A\PM^\top) = f(\X, \A)\);
    \(f(\PM\X, \PM\A\PM^\top) = \PM f(\X, \A)\)
\end{definition}

\begin{definition}[Neighborhood]
    \(\Normal_i := \{j : (i, j) \in E\}\), \(\X_{(\Normal_i)} = \multiset{\x_i: j \in \Normal_i}\) (multiset)
\end{definition}
...
\begin{theorem}
    \([\LM \vb f]_i = [\B^\top \A_e \B \vb f] = \sum_{j \in \Normal_i} a_{ij}(f_i - f_j)\) and \(\vb f^\top \LM \vb f = \norm{\grad \vb f}^2 = \sum_{ij} a_{ij} (f_i - f_j)^2\). \(\LM\) is SPSD with EV \(0 = \lambda_1 \leq \lambda_2 \leq \ldots \leq \lambda_n\).
...
\begin{definition}[Spectral Graph Convolution]
    ...
    For efficiency, use \(h(\lambda) = \sum_{k=0}^K \theta_k \lambda^k\) (Chebyshev poly.) or \(h(\lambda) = \theta (1 - \lambda)\).
...
\begin{definition}[Data Representation]
    ...
    \textbf{Propagation}: $\mathbf{X}^{l+1} = \sigma(\mathbf{W}^l \mathbf{X}^l \mathbf{Q})$ where $\mathbf{Q}$ couples node activities. Note that \(\mathbf{W}\mathbf{X}\) sum over channels, \(\mathbf{X}\mathbf{Q}\) over nodes.
\end{definition}
...
\begin{theorem}[Spectral Filtering]
    Using $\mathbf{Q} = \bar{\mathbf{A}}$ (normalized adjacency) is equivalent to first-order spectral filter $h(\bar{\mathbf{L}}) = \mathbf{I} - \bar{\mathbf{L}}$.
