...
\begin{definition}[Representation]
    Represent linear actions of \(\G\) via \(\rho: \G \to \R^{n \times n}\), assing invertible matrix \(\rho(\gg)\) and satisfying \(\rho(\gg\hg) = \rho(\gg) \rho(\hg)\). Again, stated \(\rho(\gg)x(u) = x(\gg^{-1} x)(u)\) and \((\rho(\gg)(\rho(\hg)x))(u) = (\rho(\gg\hg)x)(u)\).
\end{definition}
