...
\begin{definition}[SMPL LBS]
    % \(t_i' = \sum_k w_{ki}G_k(\theta, J(\beta))(t_i + s_i(\beta) + p_i(\theta))\), and final position: \(\sum_{k} w_{ji}G_k(\theta)G_k(\theta_0)^{-1}t_i'\).
    \(\bar t_i' = \sum_{k} w_{ki} G_k'(\theta, J(\beta))(\bar t_i + s_i(\beta) + p_i(\theta))\), \(G_k'(\theta, J) = G_k(\theta, J) G_k(\theta^*, J)^{-1}\)
\end{definition}
