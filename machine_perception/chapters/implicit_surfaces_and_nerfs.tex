...
\begin{definition}[Plane SDF]
    \(ax + by + cz + d = 0\)
\end{definition}

\begin{definition}[Z-aligned Torus SDF]
...
\begin{definition}[\(\alpha\)-compositing]
    \(\alpha = 1 - e^{-\sigma_i \delta_i}\), \(\delta_i = t_{i+1} - t_i\) (distance between samples).
...
\begin{definition}[Gaussian Splatting]
    For each bucket, sort Gaussians by depth. Calculate opacity: \(\alpha = o \cdot \exp(- 0.5(\x - \m')^\top \S^{'-1}(\x - \m'))\); \(o\) is gaussian opacity, \(\m', \S'\) are means and cov mats of 2D projection.
    Covariance is \(\S = RSS^\top R^\top\)
\end{definition}

\begin{definition}[VRE]
    \(C = T_1 \alpha_1 c_1 + \ldots\) and \(T_1 = 1, T_2 = 1 - \alpha_1,\) and \(T_3 = (1 - \alpha_1)(1 - \alpha_2)\), etc.
\end{definition}
