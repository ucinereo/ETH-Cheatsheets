...
Let \(\x \in \R^n\), \(\y \in \R^m\), then
\begin{itemize}
    \item \textbf{Numerator}: \(\frac{\partial \y}{\partial \x} \in \R^{m \times n}\) and \(\nabla f(\x) = \left[\frac{\partial f(\x)}{\x}\right]^\top\) (used here)
    \item \textbf{Denominator}: \(\frac{\partial \y}{\partial \x} \in \R^{n \times m}\) and \(\nabla f(\x) = \frac{\partial f(\x)}{\x}\)
\end{itemize}

\begin{definition}[Matrix Det. Lemma]
    \(\text{det}(\A + \u\v^\top) = (1 + \v^\top \A^{-1} \u)\text{det}(\A)\)
\end{definition}
...
\begin{definition}[Gauss]
    \(p(\x) = \frac{1}{(2\pi)^{m/2} \vert \S \vert^{\frac 1 2}} \exp(-\frac 1 2 (\x - \m)^\top \S^{-1} (\x - \m))\)

    \(\X\), \(\Y\) independent gauss \(\implies\) \(\Var[\X + \Y] = \Var[\X] = \Var[\Y]\).
\end{definition}
...
\begin{definition}[Lasso]
    \(\Omega(\p) = \norm{\p}_1 \to \nabla \tilde \L = \nabla \L + \lambda (-1)^{\mathbb{I}_{\{\p < 0\}}}\) (sparse)
\end{definition}
...
\begin{definition}[Transfer learning]
    If not sufficient data, train model on different task with more data available and then fine-tune.
\end{definition}

\begin{definition}[Early Stop]
    If last \(p\) epochs no validation improvement, stop.
\end{definition}
...
\begin{definition}[Nesterov's Momentum]
    ...
    \(v = \alpha v - \eta \nabla_\p\left(\frac 1 m \sum_i \L(f(x\i; \p + \alpha v), y\i)\right)\).
\end{definition}

\begin{definition}[AdaGrad]
    Keep sum of past squared gradients per dimension (\(\nabla_2\)).
    Then learning rate is: \(\eta \frac{\nabla_\p}{\alpha \nabla_2} \).
\end{definition}
