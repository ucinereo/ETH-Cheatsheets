...
\(D^* = \frac{p_{\text{data}}(\x)}{p_{\text{data}}(\x) + p_{\text{model}}(\x)}\), by setting \(D^* = D(\x)\) and opt. f. inside \(\int\).

\(G^*\) if \(p_{\text{model}}(\x) = p_{\text{data}}(\x)\) at opt. \(V(G^*, D^*) = -\log 4\) as

\(V(G, D^*) = 2 \cdot D_{JS}(p_{\text{data}} \Vert p_{\text{model}}) - \log 4\).
